...
export const searchStock = symbol => {
  return async dispatch => {
    ...
    if (Stock_detail_table_[0] === undefined) {
      ...
      dispatch({
        type: "SEARCH_STOCKS",
        payload: { symbol, Stock_detail_table }
      });
    }
  };
};
...
export const postComment = (id, comment) => {
  return dispatch => {
    ...
    this.eosio
      .transaction(process.env.REACT_APP_EOSIO_ACCOUNT, "commentpost", {
        ...obj
      })
      .then(function(value) {
        console.log(value);
...
export const tweetsFetchAPI = url => {
  ...
  return dispatch => {
    axios
      ...
      .then(res => dispatch({ type: "LOAD_TWEETS", newTweets: res }))
...
export const hitlike = (id, author, timestamp) => {
  ...
  return dispatch => {
    ...
    this.eosio
      .transaction(process.env.REACT_APP_EOSIO_ACCOUNT, "likepost", {
        ...obj
      })
      .then(function(value) {
...
async function newFunction(symbol) {
  const stockFromApi = await axios.get(
    "https://api.iextrading.com/1.0/stock/market/batch?symbols=" +
      symbol +
      "&types=price,company,chart&range=1m"
  );
